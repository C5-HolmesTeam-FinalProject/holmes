...
def get_info(links):
    ...
    print('Obtaining properties for rent, please wait...')
    for link in links:
        #Get the URL
...
def get_property_rent():
    ...
    new_connection = connection(type="arriendos")

    #Get the properties for rent
    propery_for_rent = get_info(new_connection)

    #Export the data to JSON file
    json_rent = json.dumps(propery_for_rent, ensure_ascii=False)
    
    with open(r".\json\property_rent.json", "w") as json_file:
        json_file.write(json_rent)
